...
Return this exact JSON structure:
{
  ...
  "feedback": {
    ...
    "model_answer": "<a complete model band 8-9 answer for the same question/prompt, written as a full essay>"
  }
}

Be clear, structured, and actionable. Do not claim this is an official IELTS score. Provide at least 2-3 sentence corrections. The rewrite should demonstrate band 7+ writing. The model_answer should be a complete, well-structured essay that would score band 8-9, responding to the same question/prompt as the student.`;
...
返回以下JSON结构：
{
  ...
  "feedback": {
    ...
    "model_answer": "<针对同一题目的完整8-9分范文，写成完整的作文>"
  }
}

重要：这是英语考试。以下内容必须用英语写：
- "corrected"（改正后的句子）
- "rewrite_example"（改写段落）
- "model_answer"（范文）
其余所有反馈、解释和评价请用清晰、自然、适合学生理解的中文表达。

不要声称这是官方雅思评分。请提供至少2-3个句子修改建议。改写示例应展示7分以上的写作水平。model_answer应该是一篇完整的、结构良好的范文，针对与学生相同的题目，达到8-9分水平。`;
...
export async function scoreEssay(
  ...
  let userContent = `Task Type: IELTS Writing ${taskType === "task1" ? "Task 1" : "Task 2"}`;
  if (promptText) {
    userContent += `\n\nQuestion/Prompt:\n${promptText}`;
  }
  userContent += `\n\nEssay:\n${essay}`;
